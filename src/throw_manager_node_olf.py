...
from rocat_sim.src.utils.utils import reset_robot, publish_marker_list_2gzb, publish_points, publish_special_point, find_point_A
import numpy as np
...
done_reset_impact_checker = False

def send_reset_impact_checker_signal_srv(trigger_value):
    global_printer.print_green(f"Sending reset signal to impact checker through service /reset_impact_checker_srv")
    try:
        rospy.wait_for_service('/reset_impact_checker_srv', timeout=5)
    except rospy.ROSException:
        rospy.logerr("Service '/reset_impact_checker_srv' is not available within timeout!")
        return
    try:
        service_client = rospy.ServiceProxy('/reset_impact_checker_srv', SetBool)
    ...
    global done_reset_impact_checker
    done_reset_impact_checker = response.success

def handle_robot_reach_goal_srv(req):
    send_reset_impact_checker_signal_srv(True)
...
def publish_trajectories(data):
    ...
    rospy.Service('/reset_impact_checker_srv', SetBool, handle_robot_reach_goal_srv)

    rate = rospy.Rate(120)  # 120 Hz

    # 1. reset impact checker throw servce /reset_impact_checker_srv
    send_reset_impact_checker_signal_srv(True)
    while not rospy.is_shutdown():
        for traj_idx, traj in enumerate(data):
            ...
            alpha_degree = random.uniform(-global_config.rocat_sim_conf.catch_ori_dev_deg_thres, global_config.rocat_sim_conf.catch_ori_dev_deg_thres)
            init_robot_pos = find_point_A(impact_point_z_up[0], impact_point_z_up[1], alpha_degree=alpha_degree, d=global_config.rocat_sim_conf.catch_dist)
            ...
            traj_show_gzb = np.array(traj_show_gzb)
            publish_marker_list_2gzb(traj_show_gzb, model_name="real_IP", color="green")
            ...
            global done_reset_impact_checker
            if not done_reset_impact_checker:
                # 2.1 wait for impact checker to reset
                global_printer.print_green('  Waiting for impact checker to reset')
                while not done_reset_impact_checker:
                    rospy.sleep(0.1)
                done_reset_impact_checker = False
